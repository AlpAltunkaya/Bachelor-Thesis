function [all_data] = Morse_Automated_Detection_general(nEEG_ds_cut,cuttime,subject_number) % Only put NREM sorted, 20 Hz low pass filtered data with a frame rate of 50 Hz.
%% LATEST VERSION, NO OPTIMIZATION WITH INTERVALS! optimization done only by looking at first 6 hours of recording from one mouse tho!
%detection peak set to 0.1605, window: 0-100, artifact cut: 1,2 V
%% Turn NaN into 0.
nEEG_ds=nEEG_ds_cut;
EEG_ds = nEEG_ds;
...
cuttime_val=cuttime;
xlswrite(['C:/Users/ALP/Desktop/lpf general output/postexpo/Detection ','M3T',num2str(subject_number),' ',num2str(cuttime_val),'_',num2str(i)],all_data);
%% Timing
...
%     ylabel('{\bfVoltage [mV]}')
%     saveas(gcf,['C:/Users/ALP/Desktop/lpf general output/Detection ','M3T',num2str(subject_number),' ',num2str(cuttime_val),'_',num2str(i),'.jpg']);
%     close all
% end
end
