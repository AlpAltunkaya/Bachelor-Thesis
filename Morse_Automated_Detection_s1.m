function [all_data] = Morse_Automated_Detection_s1(nEEG_ds_cut,cuttime,subject_number) % Only put NREMS sorted, 20 Hz low pass filtered data with a frame rate of 50 Hz.
%% Turn NaN into 0.
nEEG_ds=nEEG_ds_cut;
EEG_ds = nEEG_ds;
...
ss_tresh = find(maxvalue>=0.169743);
...
xlswrite(['C:/Users/ALP/Desktop/lpf s1_opt output/baseline/Detection ','M3T',num2str(subject_number),' ',num2str(cuttime_val),'_',num2str(i)],all_data);
%% Timing
...
for i=1:size(spindle_int,1)
    ...
    ylabel('{\bfVoltage [mV]}')
    saveas(gcf,['C:/Users/ALP/Desktop/lpf s1_opt output/baseline/Detection ','M3T',num2str(subject_number),' ',num2str(cuttime_val),'_',num2str(i),'.jpg']);
    %%                                      CHANGE OUTPUT DIRECTORY!
    close all
end
end
